...
export const MicrosoftBackgroundLogo = () => {
  ...
  const smoothProgress = useSpring(scrollYProgress, { stiffness: 20, damping: 15, mass: 0.8, restDelta: 0.0001 });
  const containerRef = useRef(null);

  // Mouse interaction
  const mouseX = useMotionValue(0);
  const mouseY = useMotionValue(0);
  const smoothMouseX = useSpring(mouseX, { stiffness: 20, damping: 15, mass: 1 });
  const smoothMouseY = useSpring(mouseY, { stiffness: 20, damping: 15, mass: 1 });
  ...
  const rotateX = useTransform(smoothMouseY, [-1, 1], [35, -35]);
  const rotateY = useTransform(smoothMouseX, [-1, 1], [-35, 35]);
    ...
    <div ref={containerRef} className="fixed inset-0 pointer-events-none z-0 overflow-hidden flex flex-col items-center justify-center select-none bg-gradient-to-b from-transparent to-neutral-50/50 dark:to-neutral-900/50 transition-colors duration-700">
      ...
      >
        ...
        >
          ...
          {/* Realistic Solid Tiles */}
          <motion.div style={{ opacity: solidTilesOpacity, transformStyle: "preserve-3d", zIndex: 10 }} className="absolute inset-0 flex items-center justify-center">
            {/* Top-Left: Red */}
            <motion.div style={{ x: -32, y: -32, z: 20 }} className="absolute w-14 h-14 rounded-[4px] shadow-[0_15px_35px_rgba(242,80,34,0.5),inset_0_2px_4px_rgba(255,255,255,0.7),inset_0_-2px_4px_rgba(0,0,0,0.3)] bg-gradient-to-br from-[#FF7E5B] to-[#F25022]">
              <div className="absolute inset-0 bg-gradient-to-bl from-white/50 via-transparent to-transparent rounded-[4px]" />
            </motion.div>
            {/* Top-Right: Green */}
            <motion.div style={{ x: 32, y: -32, z: 20 }} className="absolute w-14 h-14 rounded-[4px] shadow-[0_15px_35px_rgba(127,186,0,0.5),inset_0_2px_4px_rgba(255,255,255,0.7),inset_0_-2px_4px_rgba(0,0,0,0.3)] bg-gradient-to-br from-[#98DE00] to-[#7FBA00]">
              <div className="absolute inset-0 bg-gradient-to-bl from-white/50 via-transparent to-transparent rounded-[4px]" />
            </motion.div>
            {/* Bottom-Left: Blue */}
            <motion.div style={{ x: -32, y: 32, z: 20 }} className="absolute w-14 h-14 rounded-[4px] shadow-[0_15px_35px_rgba(0,164,239,0.5),inset_0_2px_4px_rgba(255,255,255,0.7),inset_0_-2px_4px_rgba(0,0,0,0.3)] bg-gradient-to-br from-[#33C3FF] to-[#00A4EF]">
              <div className="absolute inset-0 bg-gradient-to-bl from-white/50 via-transparent to-transparent rounded-[4px]" />
            </motion.div>
            {/* Bottom-Right: Yellow */}
            <motion.div style={{ x: 32, y: 32, z: 20 }} className="absolute w-14 h-14 rounded-[4px] shadow-[0_15px_35px_rgba(255,185,0,0.5),inset_0_2px_4px_rgba(255,255,255,0.7),inset_0_-2px_4px_rgba(0,0,0,0.3)] bg-gradient-to-br from-[#FFCE47] to-[#FFB900]">
              <div className="absolute inset-0 bg-gradient-to-bl from-white/50 via-transparent to-transparent rounded-[4px]" />
            </motion.div>
          </motion.div>
